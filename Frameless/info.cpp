...
int Patient::getreserveddoctorID() const
{
    return reserveddoctorID;
}
void Patient::putreserveddoctorID(int id)
{
    reserveddoctorID = id;
}

QString Patient::reserveddoctorIDTostring()
{
    QString buffer = "reserveddoctorID\n";
    buffer+= QString::number(getreserveddoctorID())+QString("\n");
    return buffer;
}
...
Doctor::Doctor()
{
    ...
    //initialization of a map to store schedule.
...
std::map<QString, int> Doctor::getschedule() const
{
...
void Doctor::putschedule(std::map<QString, int> liangle)
{
...
QString Doctor::scheduleTostring()
{
    ...
    std::map<QString, int> mymap = getschedule();
    for (std::map<QString, int>::iterator it = mymap.begin(); it!=mymap.end(); it++)
    {
...
{
    QFile file;
    QString filename = QString("C:/Doctor/") + QString("Doctor_") + QString::number(doc->getbianhao())+QString(".txt");
...
{
    QFile file;
    QString filename = QString("C:/Patient/") + QString("Patient_") + QString::number(pat->getbinglihao())+QString(".txt");
    file.setFileName(filename);
    ...
    if(true == isOK)
    {
        ...
        if (pat->getreserveddoctorID()!=0)
        {
            stream << pat->reserveddoctorIDTostring();
        }
        file.close();
    }
}
...
void Storage::loadfromtxtPatient(std::string addre, Patient & pat)
{
    ...
    if(true == isOK)
    {
        while(!stream.atEnd())
        {
            ...
            //translate recoresult
            else if (str == QString("recoresult"))
            {
                str = stream.readLine();
                pat.putrecoresult(str);
            }
            //translate reserveddoctorID
            else if (str == QString("reserveddoctorID"))
            {
                str = stream.readLine();
                pat.putreserveddoctorID(str.toInt());
            }
